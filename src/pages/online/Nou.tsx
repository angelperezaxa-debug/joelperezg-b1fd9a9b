...
function NovaSala() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // Sala objectiu: la primera (en l'ordre dels botons) que no estiga plena.
        // Calculem també el codi del primer placeholder lliure d'aquesta sala
        // perquè el nom de la mesa coincideixi amb el que ja es veia al lobby.
        ...
        try {
          ...
          // Si falla, el backend triarà per hash.
        }
  ...
  if (!hasName) {
    return (
      <main className="menu-screen min-h-screen flex flex-col items-center px-5 py-8">
