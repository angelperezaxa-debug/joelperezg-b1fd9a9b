...
export function TrucBoard(props: TrucBoardProps) {
  ...
  const isFirstToPlayFirstTrick = isFirstOfPairFirstTrick && cardsInFirstTrick.length === 0;
  const isSecondWithThreeOnTable =
    isFirstOfPairFirstTrick &&
    cardsInFirstTrick.length === 1 &&
    cardsInFirstTrick[0]?.card.rank === 3;
  ...
  const highlightPreguntes =
    (((isFirstToPlayFirstTrick && hasGoodCardHL) || isSecondWithThreeOnTable) ||
      highlightPreguntesAsk2nd) &&
    !preguntesDismissed;
  const highlightedPhraseIds = new Set<ChatPhraseId>();
  if (!preguntesDismissed) {
    if (isFirstToPlayFirstTrick && hasGoodCardHL) {
      highlightedPhraseIds.add("puc-anar");
      highlightedPhraseIds.add("que-tens");
    }
    if (isSecondWithThreeOnTable) {
